...
from ouroboros.loop_llm_call import call_llm_with_retry, emit_llm_usage_event, estimate_cost, _local_display_name
        ...
        while True:
            ...
            if msg is None:
                ...
                if not fallback_model or fallback_model == active_model:
                    primary_name = _local_display_name() if active_use_local else active_model
                    local_tag = " (local)" if active_use_local else ""
                    return (
                        f"⚠️ Failed to get a response from model {primary_name}{local_tag} after {max_retries} attempts. "
                ...
                primary_name = _local_display_name() if active_use_local else active_model
                fallback_name = _local_display_name() if fallback_use_local else fallback_model
                primary_tag = " (local)" if active_use_local else ""
                fallback_tag = " (local)" if fallback_use_local else ""
                emit_progress(f"⚡ Fallback: {primary_name}{primary_tag} → {fallback_name}{fallback_tag} after empty response")
                ...
                if msg is None:
                    return (
                        f"⚠️ All models are down. Primary ({primary_name}{primary_tag}) and fallback ({fallback_name}{fallback_tag}) "
