...
def enqueue_evolution_task_if_needed() -> None:
    ...
    if consecutive_failures >= 3:
        ...
        send_with_budget(
            int(owner_chat_id),
            f"\n🧬⚠️ Evolution paused: {consecutive_failures} consecutive failures. "
            "Use /evolve start to resume after investigating the issue."
        )
        persist_queue_snapshot(reason="evolution_circuit_breaker")
        return

    # Check budget reserve
    if budget_remaining() < EVOLUTION_BUDGET_RESERVE:
        log.warning("Not enqueueing evolution: budget reserve not met ($%.4f < $%.2f)",
                   budget_remaining(), EVOLUTION_BUDGET_RESERVE)
        return

    # Build task
    cycle = int(st.get("evolution_cycle") or 1)
    st["evolution_cycle"] = cycle + 1
    save_state(st)
    ...
    send_with_budget(int(owner_chat_id), f"\n🧬 Evolution # {cycle}: {tid}")
    send_with_budget(int(owner_chat_id), f"🧬 Evolution task {tid} started.")